...
def find_lqr_control_input(cart_pole_env):
    ...
    for i in range(1, cart_pole_env.planning_steps + 1):
        P_plus_1 = Ps[i - 1]
        P = Q + A_T @ P_plus_1 @ A - \
            A_T @ P_plus_1 @ B @ (np.linalg.inv(R + B_T @ P_plus_1 @ B)) @ B_T @ P_plus_1 @ A
        K = - np.linalg.inv(R + B_T @ P_plus_1 @ B) @ B_T @ P_plus_1 @ A
        u = K @ xs[i - 1]
...
if __name__ == '__main__':
    angles = [np.pi * 0.1, 1.14, 1.14*0.5]
    theta_values = []
    ...
    for angle in angles:
        ...
        while not is_done:
            ...
            # actual_action = (Ks[iteration] * np.expand_dims(actual_state, 1)).item(0) #TODO this is the original, is there a mistake?
